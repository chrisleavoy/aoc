...
input = open(Path(__file__).parent / 'day17-input.txt').readline().strip()
...
while True:
            ...
            for y in range(0, block_h):
                ...
                for x in range(block_w):
                    if grid[block_y + y][block_x + x] == '#' or block[y][x] == '@':
                        ...
                        new_substr += '.'
                grid[block_y + y] = gstr[0:block_x] + \
                    new_substr + gstr[block_x + block_w:]
            break
    ...
    t = tuple(grid[0:bottom_height+1])
    hsh = f'{hash(t)}|{block_type}|{jet_i}'
    if hsh in hashes:
        ...
        if cycle_len < (max_blocks - num_blocks):
            ...
            print(
                f'Skipping ahead from {num_blocks}: {ncycles} cycles of length {cycle_len} with height {cycle_height}')
